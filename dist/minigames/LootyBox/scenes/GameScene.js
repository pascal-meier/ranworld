...
import { generateLoot, RARITY_COLORS } from "../systems/RNGparcel.js";
...
export class LootyBoxGameScene extends BaseScene {
    ...
    onBoxClicked(box) {
        ...
        if (won)
            ...
            phases.startCooldown(() => this.resetBoxes());
    }
    /**
    ...
    updateOddsLabels() {
        ...
        if (this.phases.getTransparencyPhase().key === "odds" && this.explainer) {
            this.boxField.showOdds((tier) => this.explainer.formatOddsForTier(tier));
        }
        else {
