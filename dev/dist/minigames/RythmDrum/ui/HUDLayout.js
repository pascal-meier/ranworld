...
export class HUDLayout {
    apply(targets, width, height) {
        ...
        const hasDestroyedText = texts.some((t) => {
            if (!t || !t.scene)
                return true;
            const sys = t.scene.sys;
            if (sys && sys.isDestroyed)
                return true;
            if (!t.frame)
                return true;
            const frameData = t.frame.data;
    ...
    computeNumbers(width, height) {
        ...
        const startWidth = Math.min(clampValue(longSide * (isPortrait ? 0.48 : 0.28), 200, 420), width - padding * 2);
    ...
    layoutPortrait(targets, width, height, numbers) {
    ...
    layoutLandscape(targets, width, height, numbers) {
        const { padding, titleFont, detailFont, statusFont, boardFont, backWidth, backHeight, startWidth, startHeight } = numbers;
