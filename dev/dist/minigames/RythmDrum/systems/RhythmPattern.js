export class RhythmPattern {
    ...
    nextPlan(phase, roundsCleared, reuseLast) {
    ...
    buildNotes(phase, roundsCleared) {
        switch (phase.key) {
            case "memoryWarmup":
    ...
    buildSyncPattern(roundsCleared) {
    ...
    buildChaosPattern(roundsCleared) {
    ...
    buildVariations(phaseKey, length) {
    ...
    evaluateChance(phaseKey, variations, noteCount) {
        if (phaseKey === "memoryWarmup")
            return 0;
        if (!variations.length)
            return 0;
        if (phaseKey === "syncShift") {
            const deviation = variations.reduce((sum, variation) => {
        ...
        const delayPressure = variations.reduce((sum, variation) => sum + Math.max(0, variation.delayFactor - 1), 0) /
            variations.length;
    ...
    describeChance(phaseKey, intensity) {
        if (phaseKey === "memoryWarmup")
            return "Chance: 0% (pure skill)";
        const percent = Math.round(intensity * 100);
        if (intensity < 0.25)
            return `Chance: ${percent}% (subtle drift)`;
        if (intensity < 0.55)
            return `Chance: ${percent}% (noticeable sway)`;
