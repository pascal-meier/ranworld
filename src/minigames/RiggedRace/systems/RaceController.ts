import type { Runner } from "../objects/Runner.js";
import type { RaceDimensions } from "./types.js";
import { BoostManager } from "./BoostManager.js";

interface RaceControllerOptions {
  tickMs?: number;
  fairnessLabelCallback?: (text: string) => void;
}

export class RaceController {
  private readonly scene: Phaser.Scene;
  private readonly runners: Runner[];
  private dimensions: RaceDimensions;
  private readonly tickMs: number;
  private readonly fairnessLabelCallback?: (text: string) => void;
  private boostManager?: BoostManager;
  private timer?: Phaser.Time.TimerEvent;
  private running = false;
  private finishHandler?: (runner: Runner) => void;

  // ℹ️ Coordinates the race updates, randomness sampling, and finish detection ℹ️
  constructor(
    scene: Phaser.Scene,
    runners: Runner[],
    dimensions: RaceDimensions,
  ...
  setDimensions(dimensions: RaceDimensions): void {
    this.dimensions = dimensions;
  }
  ...
  private complete(runner: Runner): void {
    this.stop();
    this.finishHandler?.(runner);
  }

  // ℹ️ Updates the fairness label to show how extreme the last variance was ℹ️
  private updateFairnessLabel(extent: number): void {
    if (!this.fairnessLabelCallback) return;
