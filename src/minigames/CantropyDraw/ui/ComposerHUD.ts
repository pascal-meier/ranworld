...
import type { BoundaryMode, SymmetryMode } from "../types.js";
...
interface HUDCallbacks {
  ...
  onUndo: Callback;
  onRedo: Callback;
  onVariation: Callback;
  onPatternShift: Callback;
  onSymmetryChange: (mode: SymmetryMode) => void;
  onNoiseChange: (value: number) => void;
  onColorDriftChange: (value: number) => void;
  onBoundariesToggle: (mode: BoundaryMode) => void;
}

const SYMMETRY_ORDER: SymmetryMode[] = ["off", "vertical", "horizontal", "radial"];
const BOUNDARY_ORDER: BoundaryMode[] = ["clamp", "bleed"];
...
export class ComposerHUD {
  ...
  private newCompositionButton: Button;
  private undoButton: Button;
  private redoButton: Button;
  private variationButton: Button;
  private patternShiftButton: Button;
  private symmetryButton: Button;
  private boundaryButton: Button;
  private noiseSlider: ValueSlider;
  private colorSlider: ValueSlider;
  private root: Phaser.GameObjects.Container;
  private currentSymmetry: SymmetryMode = "radial";
  private currentBoundary: BoundaryMode = "clamp";
  private variationIndex = 0;

  constructor(private scene: Phaser.Scene, private callbacks: HUDCallbacks) {
    ...
    this.flowButton = new Button(scene, width * 0.9, height * 0.06, "Flow Off", () =>
      this.callbacks.onFlowToggle()
    );
    ...
    this.newCompositionButton = new Button(
      scene,
      width * 0.65,
      height * 0.06,
      "New Comp",
      () => this.callbacks.onNewComposition()
    );
    ...
    this.variationButton = new Button(scene, width * 0.5, height * 0.88, "Variation", () =>
      this.callbacks.onVariation()
    );
    this.patternShiftButton = new Button(scene, width * 0.65, height * 0.88, "Pattern Shift", () =>
      this.callbacks.onPatternShift()
    );
    this.symmetryButton = new Button(scene, width * 0.8, height * 0.88, "Sym: Radial", () =>
      this.cycleSymmetry()
    );
    this.boundaryButton = new Button(scene, width * 0.5, height * 0.82, "Clamp", () =>
      this.cycleBoundary()
    );
  ...
  setColorDrift(value: number): void {
  ...
  layout(width: number, height: number): void {
    ...
    [this.replayButton, this.newSeedButton, this.newCompositionButton].forEach((btn) =>
      btn.setButtonSize(topButtonWidth, topButtonHeight)
    );
