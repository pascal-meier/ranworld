export interface ValueSliderConfig {
  label: string;
  initial?: number;
  width?: number;
...
export class ValueSlider extends Phaser.GameObjects.Container {
  ...
  private trackPointerActive = false;

  constructor(scene: Phaser.Scene, x: number, y: number, config: ValueSliderConfig) {
    super(scene, x, y);
  ...
  private tagAsHud(tag = "hud-ui"): void {
    this.setDataEnabled();
